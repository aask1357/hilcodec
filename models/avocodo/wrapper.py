...
class ModelWrapper(AudioModelWrapper):
    def __init__(self, hps, train=False, rank=0, device='cpu'):
        ...
        for p in self.disc.parameters():
            num_params += p.numel()
        print(f"Number of parameters in Discriminator: {num_params}")
        ...
        self.mel_loss = MelLoss(
            ...
            hps.data.n_fft, hps.data.num_mels, self.hop_size,
            hps.data.win_size,
        )
        ...
        if not train:
            ...
            self.pqmf_lv1 = PQMF(*hps.pqmf_config.lv1).cuda(rank)
            self.pqmf_lv2 = PQMF(*hps.pqmf_config.lv2).cuda(rank)
            hp = hps.train
    ...
    def vq_initialize(self, x: torch.Tensor) -> None:
        with amp.autocast(enabled=self.fp16):
            x = self._module.encoder(x)
            x, num_replaces, loss_vq = self._module.quantizer(x, n=self.num_quantizers)
    
    def ddp_bucket_initialize(self, x: torch.Tensor) -> None:
        # use batch_size=1 for memory efficiency
        with amp.autocast(enabled=self.fp16):
            wav_gs, _, loss_vq = self.model(x[:1], n=self.num_quantizers,
                                            return_list=True)
        ...
        if self.world_size > 1:
            ...
            tmp_list = [
                self.pqmf_lv2.analysis(tmp)[:, :1],
                self.pqmf_lv1.analysis(tmp)[:, :1],
                tmp
            ]
            logits, _ = self.disc(tmp_list)
            loss = x.new_zeros(1)
    ...
    def initialize(self, dataloader):
        ...
        # scaler._scale initialization
        self.scaler.scale(torch.zeros(1, device=self.device).mean())

        for batch in dataloader:
            wav_r = batch["wav"].cuda(self.rank, non_blocking=True).unsqueeze(1)
            self.vq_initialize(wav_r)
            if self.world_size > 1:
                self.ddp_bucket_initialize(wav_r)
            break
    ...
    def train_epoch(self, dataloader):
        ...
        for idx, batch in enumerate(dataloader, start=1):
            ...
            ys = [
                self.pqmf_lv2.analysis(wav_r)[:, :1],
                self.pqmf_lv1.analysis(wav_r)[:, :1],
                wav_r
            ]
            
            with amp.autocast(enabled=self.fp16):
                wav_gs, num_replaces, loss_vq = self.model(wav_r, return_list=True)
            
            # We don't use mixed precision for Discs.
            # Note that `ys` & `wav_gs` are float.
            logits_g, fmaps_g = self.disc(wav_gs)
            logits_r, fmaps_r = self.disc(ys)
            
            # update Discriminator
            self.optim_d.zero_grad()
            loss_d = self.d_loss_fn(logits_g, logits_r)
            loss_d.backward(inputs=self.disc_parameters, retain_graph=True)
            ...
            with amp.autocast(enabled=self.fp16):
                loss_dict: Dict[str, torch.Tensor] = self.mel_loss(wav_gs[-1], wav_r)
            ...
            self.balancer.backward(loss_dict, loss_vq)
            ...
            if self.rank == 0:
                print(
                    ...
                    f"  scale {self.scaler.get_scale():.3e}"
                    f"  lr: {self.get_lr():.2e}",
                    end='', flush=True
                )
        ...
        if self.world_size > 1:
            ...
            for n, p in self._disc.named_parameters():
                ...
                for buf_from_other_rank in bucket:
                    if not torch.allclose(p.data, buf_from_other_rank):
                        print(
                            f"\rEpoch {self.epoch} - "
                            f"Train {idx:{padding}d}/{max_items} ({idx/max_items*100:>4.1f}%)"
                            f"{self.balancer.print()}"
                        ...
                        raise AssertionError(f"Discriminator-{n} is not consistent.")
        ...
        summary["scalars"]["scale"] = self.scaler.get_scale()
    ...
    def _valid_epoch(self, dataloader):
        ...
        for batch in tqdm(dataloader, desc="Valid", disable=(self.rank!=0), leave=False, dynamic_ncols=True):
            ...
            with amp.autocast(enabled=self.fp16):
                wav_gs, _, loss_vq = self._module(wav_r, n=self.infer_n, return_list=True)

                loss_dict = {'vq': loss_vq}
                loss_dict.update(self.mel_loss(wav_gs[-1], wav_r))
            logits_g, fmaps_g = self.disc(wav_gs)
            logits_r, fmaps_r = self.disc(ys)
