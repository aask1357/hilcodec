    ...
    def __init__(self, filters: int, in_channels: int = 1, out_channels: int = 1,
        ...
        self.convs.append(
            NormConv2d(spec_channels, self.filters, kernel_size=kernel_size,
                       padding=get_2d_padding(kernel_size), norm=_norm, nonlinearity='relu')
        )
...
class PQMFAvgPool(nn.Module):
    def __init__(self, stride: int, taps: int, beta: float, cutoff_freq: float):
        super().__init__()
        self.pqmf = PQMF(subbands=stride, taps=taps, beta=beta, cutoff_freq=cutoff_freq)
...
class MultiScaleDiscriminator(nn.Module):
    def __init__(self, norm: tp.Optional[str] = None, use_pqmf: bool = False):
        ...
        if use_pqmf:
            self.meanpools = nn.ModuleList([
                nn.Identity(),
                PQMFAvgPool(2, 256, 8.0, 0.25),
                PQMFAvgPool(4, 256, 8.0, 0.125),
            ])
        else:
            self.meanpools = nn.ModuleList([
                nn.Identity(),
    ...
    def forward(self, x: Tensor) -> DiscOutput:
        y_ds = []
        fmaps = []
        for idx, d in enumerate(self.discriminators):
            y = self.meanpools[idx](x)
            y_d, fmap = d(y)
            y_ds.append(y_d)
            fmaps.extend(fmap)
...
class Discriminators(nn.Module):
    ...
    def forward(self, y: Tensor) -> FinalDiscOutput:
        # y = y * self.scale
        y_ds, fmaps = {}, {}
